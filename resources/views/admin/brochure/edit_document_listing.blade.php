...
<form action="{{ route('admin.admin_brochure.update',$brochure_doc->id) }}" id="brochure-create" role="form" method="post"
    class='form-horizontal prevent-multiple-submit' files=true enctype='multipart/form-data' accept-charset="utf-8">
    @csrf
    @method('patch')
    <input type="hidden" name="brochure_id" value="{{ $brochure_doc->id}}">
                        ...
                        <table class="table table-sm table-bordered table-hover uploadTable">
                            ...
                            <tr>
                                    ...
                                    <select id="product_id" name="product_id" class="form-control form-control-sm">
                                        ...
                                        @foreach ($approved_prods as $pval)
                                            {{-- @if(!in_array($pval->p_id,$arr_brochure_product_id)) --}}
                                                <option value="{{$pval->id}}" @if($brochure_doc->product_id==$pval->id) selected @endif>{{ $pval->product }}</option>
                                            {{-- @endif     --}}
                                        @endforeach 
                                    </select>
                                </td>
                                ...
                                <td><input type="file" id="doc_type" name="doc_type"  class="form-control form-control-sm valid">
                                    {{-- <a href="{{ route('admin.admin_brochure.bro_down', encrypt($brochure_doc->id)) }}" 
                                        class="btn btn-success btn-sm float-centre">
                                        Download</a> --}}
                                </td>
                                <td><input type="file" id="broch_other_doc" name="broch_other_doc"  class="form-control form-control-sm valid">
                                    {{-- <a href="{{ route('admin.admin_brochure.other_broch_down', encrypt($brochure_doc->id)) }}" 
                                        class="btn btn-success btn-sm float-centre">
                                        Download</a> --}}
                                </td>
                            </tr>
                    ...
                    <table class="table table-sm table-bordered table-hover uploadTable">
                        ...
                        <tbody class="applicant-uploads">
                            ...
                            <tr>
                                ...
                                <td class="text-center"> 
                                    <a href="{{ route('admin.admin_brochure.edit_document_listing', ['app_id' => $apps->id,'id' =>$content->id]) }}" 
...
{!! JsValidator::formRequest('App\Http\Requests\Admin\EditBrochureDocUploads','#brochure-create') !!}
@endpush