        ...
        <div class="col-md-12">
            ...
            <div class="card border-primary">
                <div class="card-header text-white bg-primary border-primary">
                    <h5> Invoice Claim Variable List</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table id="#" class="table table-sm  table-bordered table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                        $i = 1;
                                        $getCount = count($getClaimVariableData);
                                        ?>
                                        @if (!empty($getCount))
                                            @foreach ($getClaimVariableData as $gcvd)
                                                <tr>
                                                    <td>{{ $i }}</td>
                                                    <td>{{ $gcvd->qtr_id }}</td>
                                                    <td>{{ $gcvd->financial_year }}</td>
                                                    <td>{{ $gcvd->updated_by }}</td>
                                                    <td>{{ $gcvd->created_at }}</td>
                                                    <td>{{ $gcvd->remark }}</td>
                                                    <td>
                                                        @if ($gcvd->status == 'S')
                                                            <span style="color:green; font-size:14px;"> Submit</span>
                                                        @elseif($gcvd->status == 'D')
                                                    ...
                                                    <td>
                                                        @if ($gcvd->status == 'S')
                                                            <a class="btn btn-primary btn-sm" data-toggle="modal"
                                                                data-target="#exampleModal3" id="{{ $gcvd->id }}"
                                                                onClick="viewMailSchedular(this.id)"><i class="fa fa-eye"
                                                                    aria-hidden="true"> View</i></a>
                                                        @elseif($gcvd->status == 'D')
                                                            <a class="btn btn-danger btn-sm" data-toggle="modal"
                                                                data-target="#exampleModal2" id="{{ $gcvd->id }}"
                                                                onClick="editClaimVariable(this.id)"><i class="fa fa-edit"
                                                                    aria-hidden="true"> Edit</i></a>
                                                        @else
                                                            <a class="btn btn-info btn-sm" data-toggle="modal"
                                                                data-target="#exampleModal" id="{{ $gcvd->id }}"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <h5 class="modal-title " id="exampleModalLabel">Claim Variable Invoice Details</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action='{{ route('admin.claimvariable.store') }}' id="mail-schedular" name="mail-schedular"
                        role="form" method="post" class='form-horizontal prevent-multiple-submit' files=true
                        enctype='multipart/form-data' accept-charset="utf-8">
                        @csrf
                        <input type="hidden" name="claimVariableId" id="claimVariableId" value="">
                        <div class="row">
                            <div class="col-md-4">
    ...
    <div class="modal fade bd-example-modal-lg" id="exampleModal2" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action='{{ route('admin.claimvariable.update') }}' id="mail-schedular" name="mail-schedular"
                        role="form" method="post" class='form-horizontal prevent-multiple-submit' files=true
                        enctype='multipart/form-data' accept-charset="utf-8">
                        @csrf
                        <input type="hidden" name="edit_claimVariableId" id="edit_claimVariableId" value="">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="exampleFormControlSelect1">Current Quarter</label>
...
@push('scripts')
    ...
    <script>
        function GFG_click(clicked) {
            ...
            $('#claimVariableId').val(clicked);
            $('#edit_claimVariableId').val(clicked);
            //document.getElementById("schedular_mail_id").value = clicked;

        }

        // below js code to open modal edit button
        function editClaimVariable(getClickedValue) {

            var id = getClickedValue;
            $('#edit_claimVariableId').val(getClickedValue);

            $.ajax({
                type: 'get',
                url: '/admin/claimvariable/' + id,
                data: {
                    id: getClickedValue
                },
                dataType: 'json',
        ...
        function viewMailSchedular(getClickedValue) {
            ...
            $.ajax({
                type: 'get',
                url: '/admin/claimvariable/view/' + id,
                // data: {
                //     id: getClickedValue
                // },
                dataType: 'json',
    ...
    <script>
        $(document).ready(function() {
            $("#finalSubmit").click(function(event) {
                var a = $("#edit_claimVariableId").val();
                event.preventDefault();
                var links = '/admin/claimvariable/finalsubmit';
                swal({
    ...
    {!! JsValidator::formRequest('App\Http\Requests\ClaimVariableSchedular', '#mail-schedular') !!}
@endpush
